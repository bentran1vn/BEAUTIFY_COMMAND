    ...
    : ICommandHandler<CONTRACT.Services.Wallets.Commands.ProcessWithdrawalRequestCommand>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (walletTransaction == null) return Result.Failure(new Error("404", ErrorMessages.Wallet.WalletNotFound));
        ...
        if (childClinic.IsFailure) return childClinic;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (childClinic == null) return Result.Failure<Clinic>(new Error("404", ErrorMessages.Clinic.ClinicNotFound));
