    ...
    CONTRACT.Services.ServicePromotions.Commands.UpdatePromotionServicesCommand>
{
    ...
    public UpdatePromotionServicesCommandHandler(IRepositoryBase<ClinicService, Guid> clinicServicerepository,
    ...
    public async Task<Result> Handle(
        CONTRACT.Services.ServicePromotions.Commands.UpdatePromotionServicesCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (isValidService == null)
            return Result.Failure(new Error("401", "Can not update promotion for this service"));
        ...
        {
            var queryLast =
                _promotionrepository.FindAll(x => x.IsActivated && !x.IsDeleted && x.LivestreamRoomId == null);
