    ...
    DeleteProcedureCommandHandler : ICommandHandler<CONTRACT.Services.Procedures.Commands.DeleteProcedureCommand>
{
    ...
    public DeleteProcedureCommandHandler(IRepositoryBase<Procedure, Guid> procedureRepository,
        IRepositoryBase<TriggerOutbox, Guid> triggerOutboxRepository, IRepositoryBase<Service, Guid> serviceRepository)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var service = await _serviceRepository.FindAll(x => x.Id.Equals(serviceId) && x.IsDeleted == false)
        ...
        foreach (var item in procedureUpdates) item.StepIndex -= 1;
