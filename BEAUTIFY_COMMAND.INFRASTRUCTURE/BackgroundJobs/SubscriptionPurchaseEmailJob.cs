    ...
    ILogger<SubscriptionPurchaseEmailJob> logger) : IJob
{
    public async Task Execute(IJobExecutionContext context)
    {
        try
        {
            ...
            logger.LogInformation("Found {count} pending subscription transactions to process", pendingTransactions.Count);
            ...
            foreach (var transaction in pendingTransactions)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.LogError(ex, "Error sending subscription purchase confirmation email for transaction {transactionId}",
                        transaction.Id);
                }
            }
